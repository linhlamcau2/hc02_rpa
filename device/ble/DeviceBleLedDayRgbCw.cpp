...
DeviceBleLedDayRgbCw::DeviceBleLedDayRgbCw(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
...
void DeviceBleLedDayRgbCw::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleLedDayRgbCw::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleLedDayRgbCw::Do(int id, int value)
{
...
bool DeviceBleLedDayRgbCw::Do(Json::Value &dataValue)
{
	bool isIdHue = false;
