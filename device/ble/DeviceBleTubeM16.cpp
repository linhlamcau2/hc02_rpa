...
DeviceBleTubeM16::DeviceBleTubeM16(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_TUBE_M16, version)
{
...
bool DeviceBleTubeM16::CheckAddr(uint32_t addr)
{
	return ((this->addr <= addr) && (this->addr + 1 >= addr));
}

int DeviceBleTubeM16::BuildTelemetryValue(Json::Value &pushDataValue)
{
...
void DeviceBleTubeM16::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleTubeM16::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleTubeM16::Do(int id, int value)
{
...
bool DeviceBleTubeM16::Do(Json::Value &dataValue)
{
