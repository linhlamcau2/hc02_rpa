...
DeviceBleFlood::DeviceBleFlood(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_FLOOD, version)
{
...
void DeviceBleFlood::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleFlood::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleFlood::Do(int id, int value)
{
...
bool DeviceBleFlood::Do(Json::Value &dataValue)
{
