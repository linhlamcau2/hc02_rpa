...
DeviceBleTrackLight::DeviceBleTrackLight(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_TRACKLIGHT, version)
{
...
bool DeviceBleTrackLight::CheckAddr(uint32_t addr)
{
	return ((this->addr <= addr) && (this->addr + 1 >= addr));
}

int DeviceBleTrackLight::BuildTelemetryValue(Json::Value &pushDataValue)
{
...
void DeviceBleTrackLight::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleTrackLight::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleTrackLight::Do(int id, int value)
{
...
bool DeviceBleTrackLight::Do(Json::Value &dataValue)
{
