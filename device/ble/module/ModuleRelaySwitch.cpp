...
void ModuleRelaySwitch::InitAttribute(int id, double value)
{
	if (this->id == id)
		this->value = value;
}
...
int ModuleRelaySwitch::InputData(uint8_t *data, int len, Json::Value &jsonValue)
{
	...
	else if (data[0] == 0x52)
	{
		...
		if (data_message1->header == 0x000e || data_message1->header == 0x000d || data_message1->header == 0x000c || data_message1->header == 0x000b)
		{
			...
			{
				this->value = data_message1->value;
#ifdef CONFIG_SAVE_ATTRIBUTE
				SaveAttribute();
#endif
				BuildTelemetryValue(jsonValue);
...
{
#ifdef CONFIG_USE_MESSAGE_FORMAT_V2
	jsonValue[KEY_ATTRIBUTE_BUTTON] = value;
#else
