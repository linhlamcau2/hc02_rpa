...
{
	database->DeviceAttributeAdd(device, key, status);
}
#endif
...
int ModuleStatusStartupRelay::Do(Json::Value &dataValue)
{
	...
	if (bleProtocol && dataValue.isObject() && dataValue.isMember(key) && dataValue[key].isInt())
	{
		int status = dataValue[key].asInt();
		if (bleProtocol->ConfigStatusStartupRelay(addr, this->index + 1, status) == CODE_OK)
			return CODE_OK;
	}
	return CODE_ERROR;
}
