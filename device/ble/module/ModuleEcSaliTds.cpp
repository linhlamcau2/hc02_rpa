...
void ModuleEcSaliTds::SaveAttribute(string key)
{
    if (key == KEY_ATTRIBUTE_SALI)
        database->DeviceAttributeAdd(device, KEY_ATTRIBUTE_SALI, sali);
    else if (key == KEY_ATTRIBUTE_EC_WATER)
        database->DeviceAttributeAdd(device, KEY_ATTRIBUTE_EC_WATER, ec);
    else if (key == KEY_ATTRIBUTE_TDS)
        database->DeviceAttributeAdd(device, KEY_ATTRIBUTE_TDS, tds);
}
#endif
