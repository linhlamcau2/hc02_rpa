...
void ModuleModeInput::InitAttribute(int id, double value)
{
    if (this->id == id)
        mode = value;
}

void ModuleModeInput::SaveAttribute()
{
...
int ModuleModeInput::InputData(uint8_t *data, int len, Json::Value &jsonValue)
{
    if (data[0] == RD_OPCODE_CONFIG_RSP)
    {
        ...
        if (data_message->header == RD_OPCODE_CONFIG_MODE_INPUT_SWITCHONOFF)
        {
