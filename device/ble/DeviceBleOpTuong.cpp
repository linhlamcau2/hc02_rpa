...
DeviceBleOpTuong::DeviceBleOpTuong(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_OP_TUONG, version)
{
...
void DeviceBleOpTuong::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleOpTuong::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleOpTuong::Do(int id, int value)
{
...
bool DeviceBleOpTuong::Do(Json::Value &dataValue)
{
