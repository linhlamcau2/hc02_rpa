...
DeviceBlePanelTron::DeviceBlePanelTron(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_PANEL_TRON, version)
{
...
bool DeviceBlePanelTron::CheckAddr(uint32_t addr)
{
	return ((this->addr <= addr) && (this->addr + 1 >= addr));
}

int DeviceBlePanelTron::BuildTelemetryValue(Json::Value &pushDataValue)
{
...
void DeviceBlePanelTron::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBlePanelTron::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBlePanelTron::Do(int id, int value)
{
...
bool DeviceBlePanelTron::Do(Json::Value &dataValue)
{
