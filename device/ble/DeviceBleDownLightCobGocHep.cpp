...
DeviceBleDownLightCobGocHep::DeviceBleDownLightCobGocHep(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_DOWNLIGHT_COB_GOC_HEP, version)
{
...
void DeviceBleDownLightCobGocHep::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleDownLightCobGocHep::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleDownLightCobGocHep::Do(int id, int value)
{
...
bool DeviceBleDownLightCobGocHep::Do(Json::Value &dataValue)
{
