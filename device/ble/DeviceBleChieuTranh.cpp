...
DeviceBleChieuTranh::DeviceBleChieuTranh(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_CHIEU_TRANH, version)
{
...
void DeviceBleChieuTranh::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleChieuTranh::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleChieuTranh::Do(int id, int value)
{
...
bool DeviceBleChieuTranh::Do(Json::Value &dataValue)
{
