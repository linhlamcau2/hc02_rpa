...
DeviceBleOpTranLoa::DeviceBleOpTranLoa(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_OP_TRAN_LOA, version)
{
...
bool DeviceBleOpTranLoa::CheckAddr(uint32_t addr)
{
	return ((this->addr <= addr) && (this->addr + 1 >= addr));
}

int DeviceBleOpTranLoa::BuildTelemetryValue(Json::Value &pushDataValue)
{
...
void DeviceBleOpTranLoa::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleOpTranLoa::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleOpTranLoa::Do(int id, int value)
{
...
bool DeviceBleOpTranLoa::Do(Json::Value &dataValue)
{
