...
DeviceBleChieuGuong::DeviceBleChieuGuong(string id, string name, string mac, string device_id, uint32_t addr, uint16_t version)
		: DeviceBle(id, name, mac, device_id, addr, BLE_LED_CHIEU_GUONG, version)
{
...
bool DeviceBleChieuGuong::CheckAddr(uint32_t addr)
{
	return ((this->addr <= addr) && (this->addr + 1 >= addr));
}

int DeviceBleChieuGuong::BuildTelemetryValue(Json::Value &pushDataValue)
{
...
void DeviceBleChieuGuong::InputData(uint8_t *data, int len, uint32_t addr)
{
	typedef struct
	{
		uint16_t u16Opcode;
...
bool DeviceBleChieuGuong::CheckData(Json::Value &dataValue, bool &rs)
{
...
bool DeviceBleChieuGuong::Do(int id, int value)
{
...
bool DeviceBleChieuGuong::Do(Json::Value &dataValue)
{
