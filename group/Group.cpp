	...
	if (bleProtocol)
	{
		for (Json::ArrayIndex i = 0; i < dataValue.size(); i++)
		{
				...
				property.isMember("VALUE") && property["VALUE"].isInt())
			{
				...
				{
					bleProtocol->SetDimmingLight(addr + ID_START, (value * 65535) / 100, TRANSITION_DEFAULT, ack);
				}
				else if (idProperty == 2)
				{
					bleProtocol->SetCctLight(addr + ID_START, (value * 192) + 800, TRANSITION_DEFAULT, ack);
				}
		...
		{
			bleProtocol->SetHSLLight(addr + ID_START, valueHue, valueSaturation, valueLuminance, TRANSITION_DEFAULT, ack);
		}
	}
	else
		LOGW("BleProtocol null");
...
void Group::DoBleV2()
{
	if (bleProtocol && dataValue.isObject())
	{
		if (dataValue.isMember(KEY_ATTRIBUTE_ONOFF) && dataValue[KEY_ATTRIBUTE_ONOFF].isInt())
		{
			...
			bleProtocol->SetOnOffLight(addr + ID_START, value, TRANSITION_DEFAULT, true);
		}
		if (dataValue.isMember(KEY_ATTRIBUTE_DIM) && dataValue[KEY_ATTRIBUTE_DIM].isInt())
		{
			...
			bleProtocol->SetDimmingLight(addr + ID_START, dim, TRANSITION_DEFAULT, true);
		}
		if (dataValue.isMember(KEY_ATTRIBUTE_CCT) && dataValue[KEY_ATTRIBUTE_CCT].isInt())
		{
			...
			bleProtocol->SetCctLight(addr + ID_START, cct, TRANSITION_DEFAULT, true);
		}
			...
			dataValue.isMember(KEY_ATTRIBUTE_LUMINANCE) && dataValue[KEY_ATTRIBUTE_LUMINANCE].isInt())
		{
			...
			bleProtocol->SetHSLLight(addr + ID_START, h, s, l, TRANSITION_DEFAULT, true);
		}
