...
int BleProtocol::CallScene(uint16_t devAddr, uint16_t scene, uint16_t transition, bool ack)
{
	...
	if (ack)
	{
		...
		int rs = SendMessage(APP_REQ, (uint8_t *)&callscene_message, sizeof(callscene_message_t), HCI_GATEWAY_RSP_OP_CODE, dataRsp, &lenRsp, 600, callSceneHeader, 0, 6);
		if (rs == CODE_OK)
		{
			typedef struct __attribute__((packed))
			{
...
{
	LOGD("Update status Relay Switch 0x%04x, type %d", devAddr, type);
	uint16_t opcode;
	switch (type)
	{
	case BLE_SWITCH_1:
		opcode = RD_OPCODE_REQUEST_STATUS_SWITCH_1;
		break;
	case BLE_SWITCH_2:
		opcode = RD_OPCODE_REQUEST_STATUS_SWITCH_2;
		break;
	case BLE_SWITCH_3:
		opcode = RD_OPCODE_REQUEST_STATUS_SWITCH_3;
		break;
	case BLE_SWITCH_4:
		opcode = RD_OPCODE_REQUEST_STATUS_SWITCH_4;
		break;
	default:
		opcode = RD_OPCODE_REQUEST_STATUS_SWITCH;
	}
	uint8_t dataRsp[100];
	int lenRsp;
	uint8_t timerHeader[] = {(uint8_t)(devAddr & 0xFF), (uint8_t)((devAddr >> 8) & 0xFF), 1, 0, 0xe3, 0x11, 0x02};
	...
	request_status_message.header = opcode;
