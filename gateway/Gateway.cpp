		...
		*/
		...
		if (logical == -1 || logical == 3 || logical == 2) // rule theo thoi gian or theo thoi gian va tb dau vao
		{
			...
			if (ruleValue.isMember("START_AT") && ruleValue["START_AT"].isString())
			{
				...
				rule = new Rule(id, 0, name, type, repeat, Util::ConvertStrTimeToInt(startAt), Util::ConvertStrTimeToInt(endAt));
				if (!rule)
				{
					LOGW("New rule error");
				}
		...
		else if (logical == 0 || logical == 1) // rule theo thiet bi dau vao, logical or
		{
			...
			rule = new Rule(id, 0, name, type, repeat);
			if (!rule)
			{
				LOGW("New rule error");
			}
		...
		ruleValue.isMember("output") && ruleValue["output"].isObject())
	{
		...
		if (ruleValue.isMember("time") && ruleValue["time"].isObject())
		{
				...
				timeValue.isMember("end") && timeValue["end"].isString())
			{
				...
				rule = new Rule(id, 0, name, type, repeat, Util::ConvertStrTimeToInt(startTime), Util::ConvertStrTimeToInt(endTime));
			}
		}
		if (!rule)
		{
