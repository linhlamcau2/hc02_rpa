...
typedef function<void(bool, bool)> OnConnectedCallbackFunc;
typedef function<void(string, string)> OnMessageCallbackFunc;
...
public:
	...
	bool isConnected();
	void OnMessageTemp(string topic, string message);
	// void SetOnMessageCallback(OnMessageCallbackFunc onMessageCallbackFunc);
	void OnConnectedTemp(bool isConnected, bool isReconnect);
	void SetOnConnectedCallback(OnConnectedCallbackFunc onConnectedCallbackFunc);
	int removeObjectFromVector(vector<MQTTPubSub *> *mqttPubSubs, MQTTPubSub *mqttPubSub);

	void makeThreadOnMessageCallback(string topic, string payload);
	void makeThreadConnectedCallback(bool isConnected, bool isReconnect = false);
...
protected:
	...
	OnMessageCallbackFunc onMessageCallbackFunc;
	OnConnectedCallbackFunc onConnectedCallbackFunc;
